...
**Note (2026-09-22):** rows for 2026-09-15 (performance, #3329/#3330), 09-16 (security, #3338/#3339), 09-17 (intelligence, #3349/#3350), 09-18 (memory, #3351/#3352), 09-19 (swarm, #3356/#3357), 09-20 (performance, #3377/#3378), and 09-21 (security, #3384/#3385) all ran (verified via `git ls-remote --heads origin "dream/*"` and `list_pull_requests`/`search_pull_requests`, not inferred — same "rows only reach `main` via PR merge" structural gap noted since 2026-08-19) but are not transcribed as rows here, to avoid another copy stuck on another unmerged branch. All 7 are OPEN/draft, all <14d old (none stale). Trailing-7-night merge rate since the 09-14/15 backlog sweep: 0/7 — worth watching but not yet actionable (no single night is stale, no repeated-direction pattern).

| 2026-09-22 | intelligence | `EWCConsolidator.updateFisherFromConfidences()` (`ewc-consolidation.ts`, wired into production at `intelligence.ts:413` via #3110) ran its online-Fisher EMA backwards vs. its two sibling `globalFisher`-updating methods in the same class (`computeFisherMatrix()`, `recordGradient()`) — new batch got 99% weight, accumulated importance got 1%, discarding ~99% of accumulated Fisher on every `distillLearning()` pass and defeating EWC++'s anti-catastrophic-forgetting purpose; flagged as lead candidate by 09-21's scan finding, independently re-verified against the code and the EWC++ literature (Chaudhry et al. ECCV 2018, arXiv:1801.10112), fixed to match the sibling sites. **Update (ruvnet evidence-gate review, same session):** initial fix REJECTed pending lifecycle hardening — no guard against non-finite input, a no-op update still decayed/persisted Fisher, no validation on loaded persisted state, receipt bound the pre-session base commit not the candidate. All addressed: non-finite samples dropped before folding in; zero-sample updates leave state untouched; `loadFromDisk()` validates/quarantines a malformed `globalFisher`; witness rebound to the hardened candidate commit; 3 new regression tests confirmed via stash isolation; also verified against the compiled `dist/` output directly (not just vitest's transform) | #3394 | #3395 | yes | ACCEPT-scoped (post-review) | 5 EMA-direction/consistency tests + 3 hardening regression tests (non-finite-input drop, no-op-no-persist, corrupted-state quarantine), all baseline-fails/candidate-passes via stash isolation; independently reproduced by adversarial critic pre-review (CONFIRMED) and by ruvnet's evidence-gate review post-review (REJECT → all items closed); full `@claude-flow/cli` suite byte-identical 96-file failure set throughout; `tsc --noEmit` byte-identical (463 errors) throughout | fc2088df... | 09-15..09-21 (7 nights) all still open/draft, none merged since the 09-14/15 backlog sweep, none stale |
